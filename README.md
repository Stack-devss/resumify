...
Resumify is a powerful open-source resume builder and resume parser.
...
Resumify's resume builder allows user to create a modern professional resume easily.
...
| **5. Successful Track Record** | Resumify users have landed interviews and offers from top companies, such as Dropbox, Google, Meta to name a few. It has been proven to work and liken by recruiters and hiring managers. |

## 🔍 Resume Parser

Resumify’s second component is the resume parser. For those who have an existing resume, the resume parser can help test and confirm its ATS readability.
...
Resumify is created with the NextJS web framework and follows its project structure. The source code can be found in `src/app`. There are a total of 4 page routes as shown in the table below. (Code path is relative to `src/app`)
